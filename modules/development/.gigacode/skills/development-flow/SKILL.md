...
условий входа — `.gigacode/stages.json`. По мере появления producer'а
`contract.json` (WI-7) в карту добавится условие `plan ← contract`; в v1 переход
`plan` (запись openspec-пропозала) намеренно не гейтится.
