...
- **contract → нужно подтверждение понимания задачи (человек).** Запись
  `docs/development/<slug>/contract.json` заблокирована, пока человек **вне
  сессии агента** не выполнит `python .gigacode/hooks/confirm.py intake <slug>`.
  Сам агент эту команду запустить не может — её режет `git_guard`.
- **delivery → нужен прошедший verdict (машина).** Запись
  `docs/development/<slug>/pr-summary.md` заблокирована, пока verifier не запишет
  `docs/development/<slug>/verdict.json` с `result: "pass"`.
...
условий входа — `.gigacode/stages.json`. По мере появления producer'ов
(`contract.json`/`verdict.json` из WI-7/WI-11) в карту добавляются новые условия;
в v1 переход `plan` (запись openspec-пропозала) намеренно не гейтится.
