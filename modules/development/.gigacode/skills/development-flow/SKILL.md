...
- **implement → пиши код в рамках scope (overshoot = ask).** Пока контракт
  заморожен (approval:contract есть), запись code-файла **вне `scope_globs`**
  даёт `ask` (`gate_scope_guard` — overshoot): подтверди расширение явно. Внутри
  scope — без вопросов. Контракт после подтверждения **заморожен**: правка
  `contract.json` блокируется (amend = пере-подтверждение), поэтому scope нельзя
  тихо расширить. Overshoot'ы журналируются и попадают в `verdict.json`
  (`overshoot_asks`, `out_of_contract_files`).
...
условий входа — `.gigacode/stages.json`. Остаток жизненного цикла контракта
(amend/restart status, журнал overshoots, HMAC-штамп) — следующая итерация поверх
`gate_scope_guard`.
