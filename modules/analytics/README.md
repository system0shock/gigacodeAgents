...
- `.gigacode/agents/` - четыре специализированных субагента: intake, mapping, documentation и review (доказательность и структура в одном проходе).
...
3. Аналитик подтверждает область анализа; подтвержденная область фиксируется в `docs/scopes/<feature-slug>.md` со строкой `Статус: подтвержден`.
4. Документация составляется в AsciiDoc на русском языке.
5. Review проверяет доказательность (неподтвержденные утверждения, противоречия, метки) и структуру за один проход.
6. Hooks валидируют запрос и результат: hook на Stop определяет текущую функцию по scope-файлу, проверяет состав файлов, AsciiDoc, русский язык и наличие меток `Источник:`/`Статус:`.
