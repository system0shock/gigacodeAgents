...
def _rel_to_root(path):
    ...
    repo-relative globs/prefixes (quality-gate globs, techdoc/final-tree paths).
    Some runtimes (e.g. Qwen on Windows) report tool paths as absolute;
    comparing those against relative patterns silently never matches. Paths
    already relative, or outside the repo, are returned unchanged."""
    if not path:
        return path
